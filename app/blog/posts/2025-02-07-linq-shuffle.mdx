...
Under the hood, it uses `Random.Shared.Shuffle()` method to randomize the elements of a collection. Just a reminder, `Random.Shared` is a thread-safe random number generator that was introduced in .NET 6. If you are used to generating random numbers using `new Random().Next()`, consider switching to `Random.Shared.Next()` as it is a better choice for most scenarios and does not require to instantiate new `Random` object.
